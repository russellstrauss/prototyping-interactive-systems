...
class Step(Frame):
	...
	def __init__(self, parent, data, stepname): # parent is tkinter container, data is a dictionary where data will be stored, stepname is unique string and is the key for data in the dictionary
	...
	def _step_completed(self):
		self.completed = True
		self.parent.event_generate("<<step_complete>>", when="tail") # send event up to parent container (which should be the wizard)
	...
	def _step_not_completed(self):
		...
		self.parent.event_generate("<<step_not_complete>>", when="tail") # send event up to parent container (which should be the wizard)

	
	def onscreen_enter(self): # The Wizard calls this for the Step when it comes into view
		pass

	def onscreen_exit(self): # The Wizard calls this for the Step when it is removed from view
		pass
...
class Wizard(Frame):
	...
	def show_step(self, step):
		...
		if step < len(self.steps):
			if self.current_step is not None: # remove current step and tell the step it is going off screen
			...
			self.current_step.onscreen_enter() # Tell the step it's going up on the screen
		...
		if len(self.steps) != 0:
			...
			elif step == len(self.steps) - 1: # last
